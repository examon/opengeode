...
'''.format(pr=process_name)
    ...
    statelist = ', '.join(name for name in process.mapping.keys()
...
package {process_name} is'''.format(process_name=process_name,
    ...
    for signal in process.input_signals + [
        ...
        for state in process.mapping.keys():
            ...
            for each in reversed(exitlist):
                if trans and all(each.startswith(trans_st)
                    ...
                    taste_template.append('p{sep}{ref}{sep}exit;'
            ...
            if input_def:
                for inp in input_def.parameters:
                    ...
                    taste_template.append('{ctxt}.{inp} := {tInp}.all;'
                                          .format(ctxt=LPREFIX,
                                                  inp=inp,
                                                  tInp=param_name))
        ...
        taste_template.append('end {};'.format(signal['name']))
    ...
    for signal in process.output_signals:
        ...
        if 'type' in signal:
            typename = type_name(signal['type'])
            param_spec = '({pName}: access {sort}{shared})' \
        ...
        if simu:
            ...
            ads_template.append('type {}_T is access procedure{};'
    ...
    for proc in (proc for proc in process.procedures if proc.external):
        ...
        for param in proc.fpar:
            ...
            params.append('{par[name]}: access {sort}'
                          .format(par=param,
                                  sort=typename))
        if params:
            ri_header += '(' + ';'.join(params) + ')'
        ads_template.append(
                        '--  Sync required interface "' + proc.inputString)
    ...
    for timer in process.timers:
        ads_template.append('--  Timer {} SET and RESET functions'
                            .format(timer))
        if simu:
            # Declare callback registration for the SET and RESET functions
            ads_template.append('type SET_{}_T is access procedure'
                                 '(name: chars_ptr; duration: Integer);'
                                .format(timer))
            ads_template.append('type RESET_{}_T is access procedure'
            ...
            if tr.terminator.kind == 'next_state':
                if tr.terminator.inputString.strip() != '-':
                    ...
                    if tr.terminator.next_id == -1:
                        code.append('{ctxt}.state := {nextState};'
                                  .format(ctxt=LPREFIX,
...
def _inner_procedure(proc, **kwargs):
    ...
    pi_header = '{kind} {sep}{proc_name}{ret}'.format(kind='procedure'
    ...
    if proc.exported and not proc.external:
        local_decl.append('pragma export'
    ...
    if proc.external:
        ...
        for var_name, (var_type, def_value) in proc.variables.items():
            ...
            code.append('{name} : aliased {sort}{default};'
        ...
        code.append('end p{sep}{procName};'.format(sep=UNICODE_SEP,
